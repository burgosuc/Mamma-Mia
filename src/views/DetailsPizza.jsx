...
const DetailsPizza = () => {
  ...
  const changeCheckout = (pizza) => {
    if (pizza.selected === 0) {
      ...
      pizza.selected += 1
      window.alert('Su 🍕 se añadió al carro de compras')
    } else {
      window.alert('Esta pizza ya ha sido agregada. Diríjase al carro para aumentar el número de pizzas de este tipo.')
    }
  }
  return (
    <>
      ...
      <div className='containerDetails'>
        ...
        <div>
          <h1>{details.name}</h1>
          <p className='detailDescrip'>{details.description}</p>
          <p>Ingredientes:</p>
          <ul>
      ...
      <Button as={Link} to='/Home' variant='success' className='btnVolver'>Volver 🏠</Button>
    </>
  )
}
